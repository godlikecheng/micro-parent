...
/**
 *  ExcelTest => JSON数据转Excel
 */
public class TableXlsProcesser extends XlsProcesser {  // XLsProcesser.java
    ...
    protected ClientAnchor drawContent(XlsProcesserContext context) {
        ...
        for (int r = 0; r < tData.size(); r++) {
            ...
            for (int c = 0; c < tData.getJSONArray(r).size(); c++) {
                ...
                mergeRegion.add(
                        new CellRangeAddress(row.getRowNum(), row.getRowNum(), colStart, colStart + deltaSpan - 1));
                // if (deltaSpan - 1 != 0) {
                // context.getBoardSheet().addMergedRegion(new
                // CellRangeAddress(row.getRowNum(), row.getRowNum(), colStart,
                // colStart + deltaSpan - 1));
                // }
        ...
        for (int col = columnHeaderCellIdx.size() - 1; col >= 0; col--) {
            for (int r = context.getR1(); r <= tAnchor.getRow2(); r++) {
                HCell hCell = new HCell(context, mergeRegion,
                        context.getBoardSheet().getRow(r).getCell(columnHeaderCellIdx.get(col)));
                if (mergeKey != null) {
        ...
        for (int row = rowHeader; row >= 0; row--) {
            for (int c = 0; c < columnDataCellIdx.size(); c++) {
                VCell vCell = new VCell(context, mergeRegion,
                        context.getBoardSheet().getRow(context.getR1() + row).getCell(columnDataCellIdx.get(c)));
                if (mergeKey != null) {
        ...
        mergeCellHelper(mergeRegion, mergedList);
        mergeRegion.stream().filter(e -> e.getFirstColumn() != e.getLastColumn() || e.getFirstRow() != e.getLastRow())
                .forEach(e -> context.getBoardSheet().addMergedRegion(e));
        return tAnchor;
    }
    ...
    private void mergeCellHelper(List<CellRangeAddress> mergeRegion, List<CellHelper> mergeList) {
        ...
        CellRangeAddress[] toMerged = mergeList.stream().map(e -> e.getMergedCell()).collect(Collectors.toList())
    ...
    private abstract class CellHelper {
        ...
        public CellHelper(XlsProcesserContext context, List<CellRangeAddress> mergeRegion, Cell cell) {
            Optional<CellRangeAddress> r = mergeRegion.stream()
                    .filter(c -> c.isInRange(cell.getRowIndex(), cell.getColumnIndex())).findFirst();
